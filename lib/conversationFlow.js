...
const STEPS = [
  ...
  },
  {
    ...
    question: "How far do you need to go out of the gate?",
    explanation: "Tell us about your ESG compliance goals and requirements. Are you pursuing specific certifications, meeting tender requirements, or building long-term sustainability?",
    quickReplies: null,
    validate: (value) => {
      if (!value || value.trim().length < 5) {
        return { valid: false, message: "Please describe your compliance goals - this helps us recommend the right path forward." };
      }
      return { valid: true, value: value.trim() };
    },
    successMessage: "Got it! That helps me understand your needs. 🎯"
  },
  {
    field: 'uk_government_buyer',
    question: "Is the buyer a UK central government/NHS body (or prime supplier to them)?",
    explanation: "UK government and NHS contracts often have specific ESG requirements. This helps us tailor our recommendations.",
    quickReplies: [
  ...
  {
    field: 'high_value_contract',
    question: "Is the annual contract value ≥ £/€/$ 5m OR is the buyer a Tier-1 multinational with formal supplier ESG ratings?",
    explanation: "High-value contracts and Tier-1 multinationals typically require more comprehensive ESG documentation and certifications.",
    quickReplies: [
      { label: "✅ Yes", value: "yes", icon: "✅" },
      { label: "❌ No", value: "no", icon: "❌" }
    ],
  ...
  {
    field: 'critical_role',
    question: "Is your role critical / high-impact strategic (services)?",
    explanation: "Critical or strategic suppliers often face stricter ESG requirements and compliance standards.",
    quickReplies: [
      { label: "✅ Yes", value: "yes", icon: "✅" },
      { label: "❌ No", value: "no", icon: "❌" }
    ],
  ...
  {
    field: 'interested_bcorp',
    question: "Are you interested in B Corp Certification?",
    explanation: "🏆 B Corp Certification is a prestigious recognition for companies meeting high standards of social and environmental performance, accountability, and transparency. We offer an introductory bundle at £2,400 including gap analysis, 8×2-hour expert sessions, and corrective action plan.",
    quickReplies: [
      { label: "✅ Yes, very interested", value: "yes", icon: "✅" },
      { label: "🤔 Maybe, want to learn more", value: "maybe", icon: "🤔" },
      { label: "❌ Not at this time", value: "no", icon: "❌" }
    ],
    validate: (value) => {
      const normalized = value.toLowerCase().trim();
      if (normalized.includes('yes') || normalized.includes('interested') || normalized === '✅ yes, very interested') {
        return { valid: true, value: 'yes' };
      }
      if (normalized.includes('maybe') || normalized.includes('learn') || normalized === '🤔 maybe, want to learn more') {
        return { valid: true, value: 'maybe' };
      }
      if (normalized.includes('no') || normalized.includes('not') || normalized === '❌ not at this time') {
        return { valid: true, value: 'no' };
      }
      return { valid: true, value: normalized };
    },
    successMessage: "Thanks for letting me know! 👍"
  },
  {
    field: 'interested_ecovadis',
    question: "Are you interested in EcoVadis Certification?",
    explanation: "⭐ EcoVadis is a globally recognized sustainability rating platform. We're an EcoVadis Approved Training Partner and can help you achieve Bronze, Silver, or Gold rating. Our introductory bundle is £2,400 including gap analysis, 8×2-hour expert sessions, and corrective action plan.",
    quickReplies: [
      { label: "✅ Yes, very interested", value: "yes", icon: "✅" },
      { label: "🤔 Maybe, want to learn more", value: "maybe", icon: "🤔" },
      { label: "❌ Not at this time", value: "no", icon: "❌" }
    ...
    successMessage: "Perfect! I've noted your interest. 🌟"
  },
  {
    field: 'pain_points',
...
export function buildInterviewPayload(collectedData) {
  ...
  return {
    ...
    compliance_goal: collectedData.compliance_goal || '',
    uk_government_buyer: collectedData.uk_government_buyer || 'no',
    high_value_contract: collectedData.high_value_contract || 'no',
    critical_role: collectedData.critical_role || 'no',
    interested_bcorp: collectedData.interested_bcorp || 'no',
    interested_ecovadis: collectedData.interested_ecovadis || 'no',
