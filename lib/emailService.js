...
async function sendLeadNotificationEmail(leadData, apiResponse) {
  try {
    ...
    const msg = {
      ...
      customArgs: {
        lead_score: String(apiResponse.score || 0),
        urgency: apiResponse.urgency || 'medium',
        action: apiResponse.action || 'unknown'
      }
    };
...
async function sendBookingConfirmationEmail(bookingData) {
  try {
    if (!process.env.SENDGRID_API_KEY) {
      console.warn('SENDGRID_API_KEY not configured, skipping booking confirmation');
      return { 
        success: false, 
        error: 'SENDGRID_API_KEY not configured',
        skipped: true 
